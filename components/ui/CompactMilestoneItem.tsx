 ...
 */
export const CompactMilestoneItem: React.FC<CompactMilestoneProps> = ({
  milestone,
  index,
  type,
}) => {
  ...
  const neonColorClass = isWork
    ? 'from-cyan-500/20 to-blue-500/20'
    : 'from-purple-500/20 to-pink-500/20';
  const borderColorHover = isWork
    ? 'hover:border-cyan-500/50'
    : 'hover:border-purple-500/50';
  const titleColorHover = isWork
  ...
  return (
    <div className="relative pl-8 md:pl-10 w-full mb-8 last:mb-0">
      {/* Timeline Node Dot */}
      <div
        className={`absolute left-[3px] top-6 w-3 h-3 rounded-full ${dotColor} shadow-[0_0_10px_currentColor] z-20`}
      />
      ...
      >
        ...
        >
          ...
          <div className="flex flex-col items-center gap-4 mb-4 [transform:translateZ(20px)]">
            <div
              className={`relative w-14 h-14 md:w-16 md:h-16 shrink-0 flex items-center justify-center rounded-xl bg-card dark:bg-white border border-border/50 shadow-sm p-1.5 overflow-hidden transition-transform duration-500 group-hover:scale-105`}
            >
              {/* Back glow for logo */}
              <div
                className={`absolute inset-0 bg-gradient-to-br ${neonColorClass} opacity-0 group-hover:opacity-100 transition-opacity duration-500`}
              />
              <Image
            ...
            <div className="flex flex-col items-center text-center">
              ...
              <p className="text-sm md:text-base font-semibold text-foreground/80">
                {displayOrg} <span className="opacity-50 mx-1">&bull;</span>{' '}
                {displayLoc}
              </p>
            </div>
          </div>
          ...
          {/* Title */}
          <h3
            className={`text-lg md:text-xl font-black text-foreground ${titleColorHover} transition-colors duration-500 mb-3 flex flex-wrap items-center justify-center gap-x-2 text-center [transform:translateZ(30px)]`}
          >
            {typeof displayTitle === 'string' && displayTitle.includes('•')
              ? displayTitle.split('•').map((part, i, arr) => (
                  <React.Fragment key={i}>
                    ...
                    {i < arr.length - 1 && (
                      <span
                        className={`w-1.5 h-1.5 rounded-full ${dotColor} shadow-[0_0_8px_currentColor] shrink-0`}
                      />
                    )}
                  </React.Fragment>
                ))
          ...
          <ul className="space-y-2 text-muted-foreground text-sm leading-relaxed mb-5 flex flex-col items-start text-left [transform:translateZ(15px)]">
            {Array.isArray(displayDesc) &&
              displayDesc.map((point: string, idx: number) => (
                <li key={idx} className="flex gap-3 items-start justify-start">
                  <span
                    className={`mt-1.5 shrink-0 w-1.5 h-1.5 rounded-full ${dotColor} opacity-60`}
                  />
                  <span>{point}</span>
                </li>
              ))}
          ...
          {/* Corner Glow */}
          <div
            className={`absolute -bottom-10 -right-10 w-32 h-32 bg-gradient-to-br ${neonColorClass} rounded-full blur-3xl opacity-0 group-hover:opacity-100 transition-opacity duration-700 pointer-events-none`}
          />
        </motion.div>
      </motion.article>
    </div>
